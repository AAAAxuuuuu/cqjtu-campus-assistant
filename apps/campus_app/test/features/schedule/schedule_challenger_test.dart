...
void main() {
  ...
  group('Challenger Empirical Tests - calculateSemester', () {
    test('Date boundary switching: Aug 1 vs July 31, Jan 1 vs Dec 31, Feb 1 vs Jan 31', () {
      ...
      expect(calculateSemester(DateTime(2027, 7, 31, 23, 59, 59)), '2026-2027-2');
    });
  ...
  group('Challenger Empirical Tests - resolveSemesterKey boundaries', () {
    test('Boundary cases: empty string "" vs null vs whitespace "  " vs padded strings', () async {
      ...
      Future<String> testResolve(String? sem, {bool listen = true}) {
        return container.read(Provider((ref) => resolveSemesterKey(ref, sem, listen: listen)));
      }
      ...
      expect(keyNull, '2026-2027-1', reason: 'null should resolve to default active semester');
      expect(keyEmpty, '2026-2027-1', reason: 'empty string should resolve to default active semester');
      expect(keyWhitespace, '2026-2027-1', reason: 'whitespace string should resolve to default active semester');
      expect(keyPadded, '2025-2026-2', reason: 'padded string should be trimmed');
    });

    test('SelectedSemester set to empty string "" vs whitespace "   " vs null', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      await container.read(selectedScheduleSemesterProvider.notifier).set(null);
      final keyFromNullSelected = await testResolve(null);
      expect(keyFromNullSelected, '2026-2027-1');
    });

    test('Custom courses provider resolution with "", null, and whitespace arguments', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      await container.read(customCoursesProvider(null).notifier).addCourse(testCourse);

      // Query using empty string "" and whitespace "  "
      final nullCourses = await container.read(customCoursesProvider(null).future);
      final emptyCourses = await container.read(customCoursesProvider('').future);
      final spaceCourses = await container.read(customCoursesProvider('   ').future);
  ...
  group('Challenger Empirical Tests - Rapid Invalidations & SharedPreferences Consistency', () {
    test('Rapid semester switching while performing CRUD on custom courses', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      for (int i = 0; i < 15; i++) {
        ...
        await container.read(customCoursesProvider(null).notifier).addCourse(course);
      }
      ...
      for (final sem in semesters) {
        final raw = prefs.getString('user_default_schedule_custom_courses_$sem');
        expect(raw, isNotNull, reason: 'Key user_default_schedule_custom_courses_$sem should exist in prefs');
      }
    });

    test('Rapid semester start invalidations update activeSemesterStartProvider correctly', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      for (int i = 1; i <= 20; i++) {
        await container.read(semesterStartProvider.notifier).set(DateTime(2026, 9, i));
      }

      final activeStart = container.read(activeSemesterStartProvider).valueOrNull;
      expect(activeStart, DateTime(2026, 9, 20));
    });

    test('Concurrent reads and writes across family providers with invalid/malformed semester keys', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      for (final key in weirdKeys) {
        final weeks = await container.read(semesterTotalWeeksProvider(key).future);
        expect(weeks, defaultSemesterTotalWeeks);

        await container.read(semesterTotalWeeksProvider(key).notifier).setWeeks(18);
        final updatedWeeks = await container.read(semesterTotalWeeksProvider(key).future);
        expect(updatedWeeks, 18);
      }
    });

    test('Verify customCoursesProvider decodes corrupted JSON gracefully without crash', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      await prefs.setString('schedule_custom_courses_2026-2027-1', '{corrupted_json: true');

      await container.read(selectedScheduleSemesterProvider.notifier).set('2026-2027-1');

      // Should not throw, should return empty list
      final courses = await container.read(customCoursesProvider(null).future);
      expect(courses, isEmpty);
    });

    test('Verify total weeks clamping works for out-of-bound values', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      await container.read(semesterTotalWeeksProvider(null).notifier).setWeeks(5);
      expect(await container.read(semesterTotalWeeksProvider(null).future), minSemesterTotalWeeks);

      // Try setting 50 weeks (above max 30)
      await container.read(semesterTotalWeeksProvider(null).notifier).setWeeks(50);
      expect(await container.read(semesterTotalWeeksProvider(null).future), maxSemesterTotalWeeks);
    });

    test('Race condition check: calling setWeeks before build finishes', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      final notifier = container.read(semesterTotalWeeksProvider('race_sem').notifier);
      await notifier.setWeeks(16);

      // Await future after setWeeks
      final result = await container.read(semesterTotalWeeksProvider('race_sem').future);
      expect(result, 16);
    });
  });
}
