...
void main() {
  ...
  group('Empirical Challenge Task 1: Rapid Multi-Semester Switches (5+ Semesters)', () {
    test(
      'Sequential CRUD and rapid switching across 10 semesters with zero data leakage',
      () async {
        ...
        container
            ...
            .set('user_empirical', 'pass');

        final semesters = List.generate(
          10,
          (i) => '${2020 + i}-${2021 + i}-${(i % 2) + 1}',
        );
        ...
        for (int step = 0; step < 50; step++) {
          ...
          final currentCourses = await container.read(
            customCoursesProvider(null).future,
          );
          final currentWeeks = await container.read(
          ...
          expect(
            ...
            hasLength(2),
            reason:
                'Semester $sem must have exactly 2 courses after rapid switch step $step',
          );
          ...
          expect(
            ...
            12 + index,
            reason:
                'Semester $sem must retain its set weeks count (${12 + index}) after rapid switch',
          );
        }
        ...
        for (final key in allKeys) {
          expect(
            key.contains('schedule_custom_courses_default'),
            isFalse,
            reason: 'Key $key contains old "default" fallback',
          );
          expect(
            key.contains('schedule_total_weeks_default'),
            isFalse,
            reason: 'Key $key contains old "default" fallback',
          );
          expect(
            key.contains('null'),
            isFalse,
            reason: 'Key $key contains "null" string fallback',
          );
          expect(
            key.endsWith('_'),
            isFalse,
            reason: 'Key $key ends with empty prefix suffix',
          );
        }
      },
    );
    ...
    test(
      'Rapid concurrent switching without awaiting notifier set completion',
      () async {
        final container = ProviderContainer();
        addTearDown(container.dispose);
        ...
        final futures = semesters.map(
          (s) =>
              container.read(selectedScheduleSemesterProvider.notifier).set(s),
        );
        await Future.wait(futures);

        // Final state should be the last setting ('2025-2026-1')
        final finalSem = await container.read(
          selectedScheduleSemesterProvider.future,
        );
        expect(finalSem, '2025-2026-1');

        final key = await resolveSemesterKey(
  ...
  group(
    'Empirical Challenge Task 2: Semester Start Date Modifications & Boundaries',
    () {
      ...
      test(
        'Month and Year-End Boundaries: July 31/Aug 1, Dec 31/Jan 1, Jan 31/Feb 1',
        () {
          // July 31 -> Spring (month 7), Aug 1 -> Fall (month 8)
          expect(
            calculateSemester(DateTime(2026, 7, 31, 23, 59, 59)),
            '2025-2026-2',
          );
          expect(
            calculateSemester(DateTime(2026, 8, 1, 0, 0, 0)),
            '2026-2027-1',
          );
          ...
          expect(
            calculateSemester(DateTime(2027, 1, 1, 0, 0, 0)),
            '2026-2027-1',
          );
          ...
          expect(
            calculateSemester(DateTime(2027, 2, 1, 0, 0, 0)),
            '2026-2027-2',
          );
        },
      );
      ...
      test(
        'Dynamic semesterStartProvider modification toggles custom course views across boundaries',
        () async {
          final container = ProviderContainer();
          addTearDown(container.dispose);
          ...
          await container
              .read(customCoursesProvider(null).notifier)
              .addCourse(courseSpring);

          var courses = await container.read(
            customCoursesProvider(null).future,
          );
          ...
          expect(
            ...
            isEmpty,
            reason:
                'Aug 1 is Fall semester 2026-2027-1, Spring courses should not be visible',
          );
      ...
      test(
        'Interaction between selectedScheduleSemesterProvider and activeSemesterStartProvider',
        () async {
          final container = ProviderContainer();
          addTearDown(container.dispose);
          ...
          await container
              .read(semesterStartForKeyProvider('2027-2028-1').notifier)
              .set(DateTime(2027, 9, 6));

          final activeForSelected = container
              .read(activeSemesterStartProvider)
              .valueOrNull;
          ...
          await container
              .read(semesterStartProvider.notifier)
              .set(DateTime(2026, 10, 1));

          final activeUnchanged = container
              .read(activeSemesterStartProvider)
              .valueOrNull;
  ...
  group(
    'Empirical Challenge Task 3: Custom Courses CRUD, Semester Weeks & Stale Key Fallbacks',
    () {
      test(
        'Adding and removing custom courses across explicit family arguments vs null',
        () async {
          final container = ProviderContainer();
          addTearDown(container.dispose);
          ...
          await container
              .read(customCoursesProvider('2026-2027-2').notifier)
              .addCourse(course1);

          final listExplicit = await container.read(
            customCoursesProvider('2026-2027-2').future,
          );
          ...
          final listNullStillSem1 = await container.read(
            customCoursesProvider(null).future,
          );
          ...
          expect(
            await container.read(customCoursesProvider('2026-2027-2').future),
            hasLength(1),
          );
        },
      );
      ...
      test(
        'Semester total weeks bounds enforcement (min 12, max 30, default 20)',
        () async {
          final container = ProviderContainer();
          addTearDown(container.dispose);
      ...
      test('Absence of stale key fallbacks in SharedPreferences', () async {
        ...
        expect(
          keys.contains('user_default_schedule_total_weeks_2026-2027-1'),
          isTrue,
        );
        expect(
          keys.contains('user_default_schedule_custom_courses_2026-2027-1'),
          isTrue,
        );
