...
void main() {
  ...
  group('M2 Challenger Edge Case Tests', () {
    test(
      '1. Key generation logic under unusual account IDs (special chars, long strings, whitespace)',
      () async {
        ...
        expect(userScopedKey('中文账号_123', 'test_key'), 'user_中文账号_123_test_key');
    ...
    test(
      '2. Persistence across simulated app restart / provider disposal',
      () async {
        ...
        expect(
          prefs.getBool('user_${specialUser}_schedule_sunday_first'),
          isTrue,
        );
        expect(
          prefs.getInt('user_${specialUser}_schedule_total_weeks_2026-2027-1'),
          22,
        );
        ...
        container2
            .read(credentialsProvider.notifier)
            .set(longUser, 'pass_long');

        final longUserSundayFirst = await container2.read(
          scheduleSundayFirstProvider.future,
        );
        expect(
          longUserSundayFirst,
          isFalse,
          reason: 'Long user should get default preference',
        );
        ...
        expect(
          prefs.getBool('user_${longUser}_schedule_sunday_first'),
          isFalse,
        );
        ...
        container2
            .read(credentialsProvider.notifier)
            .set(specialUser, 'password123');
        final recheckedSundayFirst = await container2.read(
          scheduleSundayFirstProvider.future,
        );
        expect(
          recheckedSundayFirst,
          isTrue,
          reason: 'Special user preference must remain intact',
        );
      },
    );
  });
