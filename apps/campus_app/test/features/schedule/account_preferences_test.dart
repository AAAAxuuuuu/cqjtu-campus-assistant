...
void main() {
  ...
  group('R2 Account ID Preference & Data Isolation Tests', () {
    test('1. Schedule Sunday First preference is strictly isolated per account ID', () async {
      ...
      await container
          ...
          .setSundayFirst(true);
      final sundayFirstA = await container.read(scheduleSundayFirstProvider.future);
      ...
      final sundayFirstB = await container.read(scheduleSundayFirstProvider.future);
      expect(sundayFirstB, isFalse, reason: 'Account B must not inherit Account A preference');
      ...
      container.read(credentialsProvider.notifier).set('account_a', 'pass_a');
      final restoredSundayFirstA = await container.read(scheduleSundayFirstProvider.future);
      expect(restoredSundayFirstA, isTrue, reason: 'Account A preference must be restored intact');
    });

    test('2. Semester Total Weeks is strictly isolated per account ID and semester', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      final weeksA = await container.read(semesterTotalWeeksProvider(null).future);
      expect(weeksA, 24);

      final prefs = await SharedPreferences.getInstance();
      expect(prefs.getInt('user_user_1001_schedule_total_weeks_2026-2027-1'), 24);

      // Switch to Account B
      container.read(credentialsProvider.notifier).set('user_1002', 'pass2');
      final weeksB = await container.read(semesterTotalWeeksProvider(null).future);
      expect(weeksB, defaultSemesterTotalWeeks, reason: 'Account B should get default total weeks');

      // Account B sets 16 weeks
      await container.read(semesterTotalWeeksProvider(null).notifier).setWeeks(16);
      expect(prefs.getInt('user_user_1002_schedule_total_weeks_2026-2027-1'), 16);

      // Switch back to Account A
      container.read(credentialsProvider.notifier).set('user_1001', 'pass1');
      final restoredWeeksA = await container.read(semesterTotalWeeksProvider(null).future);
      expect(restoredWeeksA, 24, reason: 'Account A total weeks must be restored intact');
    });

    test('3. Custom Courses are strictly isolated per account ID without cross-account leakage', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      final coursesA = await container.read(customCoursesProvider(null).future);
      expect(coursesA, hasLength(1));
      expect(coursesA.first.name, 'Account A Math');

      final prefs = await SharedPreferences.getInstance();
      expect(
        prefs.containsKey('user_student_a_schedule_custom_courses_2026-2027-1'),
        isTrue,
      );

      // Switch to Account B
      container.read(credentialsProvider.notifier).set('student_b', 'secret');
      final coursesB = await container.read(customCoursesProvider(null).future);
      expect(coursesB, isEmpty, reason: 'Account B must not see Account A custom courses');
      ...
      final updatedCoursesB = await container.read(customCoursesProvider(null).future);
      expect(updatedCoursesB, hasLength(1));
      expect(updatedCoursesB.first.name, 'Account B Physics');

      // Switch back to Account A
      container.read(credentialsProvider.notifier).set('student_a', 'secret');
      final restoredCoursesA = await container.read(customCoursesProvider(null).future);
      expect(restoredCoursesA, hasLength(1));
      expect(restoredCoursesA.first.name, 'Account A Math');
    });

    test('4. Dorm Room & Campus selection is strictly isolated per account ID', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      expect(readRoomB, isNull, reason: 'Account B must not inherit Account A dorm room');
    ...
    test('5. Unauthenticated / Guest fallback handles data isolation using guest namespace', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      ...
      container.read(credentialsProvider.notifier).set('user_authenticated', 'pass');
      final userSundayFirst = await container.read(scheduleSundayFirstProvider.future);
      expect(userSundayFirst, isFalse, reason: 'Logged-in user must not pollute/share guest preference');

      // Clear credentials (logout)
      container.read(credentialsProvider.notifier).clear();
      final guestSundayFirst = await container.read(scheduleSundayFirstProvider.future);
      expect(guestSundayFirst, isTrue, reason: 'Guest preference restored when logging out');
    });

    test('6. Legacy un-prefixed keys fall back gracefully when account-scoped key is absent', () async {
      final prefs = await SharedPreferences.getInstance();
      ...
      final sundayFirst = await container.read(scheduleSundayFirstProvider.future);
      expect(sundayFirst, isTrue);

      final totalWeeks = await container.read(semesterTotalWeeksProvider(null).future);
      ...
      expect(prefs.getBool('user_legacy_user_schedule_sunday_first'), isFalse);
    });
  });
}
