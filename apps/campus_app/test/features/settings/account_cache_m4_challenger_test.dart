...
void main() {
  ...
  group(
    'Milestone 4 Challenger 1 - Campus App Account Cache & Boundary Isolation Tests',
    () {
      test(
        'Boundary Isolation Defect Test: user vs user_1 preference preservation',
        () async {
          SharedPreferences.setMockInitialValues({
          ...
          expect(
            prefs.containsKey('user_user_1_schedule_sunday_first'),
            isTrue,
            reason:
                'DEFECT: clearCurrentAccountCache("user") inadvertently deleted user_1 schedule_sunday_first preference',
          );
          expect(
            prefs.containsKey('user_user_1_dorm_campus'),
            isTrue,
            reason:
                'DEFECT: clearCurrentAccountCache("user") inadvertently deleted user_1 dorm_campus preference',
          );
        },
      );

      test(
        'Boundary Isolation Defect Test: test vs test_user preference preservation',
        () async {
          SharedPreferences.setMockInitialValues({
          ...
          expect(
            prefs.containsKey('user_test_user_schedule_sunday_first'),
            isTrue,
            reason:
                'DEFECT: clearCurrentAccountCache("test") inadvertently deleted test_user preference',
          );
        },
      );

      test(
        'Boundary Isolation Pass Test: abc vs abcd preference preservation',
        () async {
          SharedPreferences.setMockInitialValues({
      ...
      test(
        'Exact Account Key Isolation: exact account key user_user_1 remains intact after clearCurrentAccountCache(user)',
        () async {
          SharedPreferences.setMockInitialValues({
          ...
          expect(
            prefs.containsKey('user_user_1'),
            isTrue,
            reason:
                'Exact account key user_user_1 MUST remain intact after clearCurrentAccountCache("user")',
          );
        },
      );

      test(
        'Exact Account Key Isolation: exact account key user_test_user remains intact after clearCurrentAccountCache(test)',
        () async {
          SharedPreferences.setMockInitialValues({
          ...
          expect(
            prefs.containsKey('user_test_user'),
            isTrue,
            reason:
                'Exact account key user_test_user MUST remain intact after clearCurrentAccountCache("test")',
          );
        },
      );

      test(
        'One-click account cache clearing for single and multiple account sessions',
        () async {
          ...
          expect(
            prefs.containsKey('user_userA_schedule_sunday_first'),
            isFalse,
          );
      ...
      test(
        'Preference persistence & restoration after cache clear operations',
        () async {
          final container = ProviderContainer();
          addTearDown(container.dispose);
          ...
          expect(
            prefs2.containsKey('user_userA_schedule_sunday_first'),
            isFalse,
          );

          // Post-clear: reading provider returns default (false)
          final postClearValue = await container.read(
          ...
          final restoredValue = await container.read(
            scheduleSundayFirstProvider.future,
          );
